...
def verify_expr_pairs(expr_pairs: list) -> tuple[int, int, list]:
    n_corrects = 0
    n_expr_pairs = 0
    incorrects = []
    ...
    return n_corrects, n_expr_pairs, incorrects


def w_incorrects(incorrects: list[tuple[str, str]], filepath: str) -> None:
    file = open(file=filepath, mode='a')

    for expr_pair in incorrects:
...
def verify(data_dir: str, incorrect_dir: str) -> None:
    ...
    for filepath in progbar:
        ...
        for line in file:
            if line.strip():
                ...
                n_corrects, n_expr_pairs, incorrects = verify_expr_pairs(expr_pairs=expr_pairs)

                tot_n_corrects += n_corrects
                tot_n_expr_pairs += n_expr_pairs
                if incorrects:
                    ...
                    w_incorrects(incorrects=incorrects, filepath=filepath)

                equiv_exprs = []

        file.close()

    logger.log_info(f"Total number of correct expression pairs   {tot_n_corrects}")
    logger.log_info(f"Total number of incorrect expression pairs {tot_n_expr_pairs-tot_n_corrects}")
    logger.log_info(f"Total number of expression pairs           {tot_n_expr_pairs}")
    logger.log_info(f"Accuracy {tot_n_corrects/tot_n_expr_pairs*100:.4f}%")

    return


def main():
    parser = argparse.ArgumentParser(prog="verify", description="Verify if all generated expressions are equivalent")
    parser.add_argument("--dataset_dir", "-d", required=True, help="dataset directory")

    args = parser.parse_args()
    dataset_dir = args.dataset_dir

    verify(data_dir=dataset_dir, incorrect_dir=config.DATA_INCORRECT_DIR)
